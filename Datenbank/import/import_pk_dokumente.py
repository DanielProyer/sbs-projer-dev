# -*- coding: utf-8 -*-
"""Laedt die PK-/BVG-Dokumente ins Dokumente-Modul (Bereich versicherungen,
Kategorie pensionskasse).
...
1. 00_Rechnungen/06_PK/aufbereitet/*.pdf - aus den 105 Handy-Fotos gebaut.
   Die Dokumentgrenzen stehen explizit in pk_dokumente_katalog.csv (Spalte
...
if ERSETZEN:
    alt = sb.table('dokumente').select('id,storage_pfad').eq('user_id', USER) \
            .eq('bereich', 'versicherungen').eq('kategorie', 'pensionskasse').execute().data
...
def lade(pfad, *, typ, jahr, datum, betrag, titel, data=None):
    ...
    sb.table('dokumente').insert({
        ...
        'kategorie': 'pensionskasse', 'jahr': jahr, 'dokument_datum': datum,
        'betrag': float(betrag) if betrag else None, 'referenz': '2/452968',
...
for r in csv.DictReader(open(HERE / 'pk_dokumente_katalog.csv', encoding='utf-8'), delimiter=';'):
    teile = [r['datum'][:4], r['aussteller'], r['detail']]
    if r['betrag']:
...
# 2) Original-PDFs aus dem Portal, Duplikate per MD5 raus
gesehen = {}
for pfad in sorted(QUELLE.glob('*.pdf')):
    data = pfad.read_bytes()
    h = hashlib.md5(data).hexdigest()
    if h in gesehen:
        print('Duplikat von', gesehen[h], '->', pfad.name); continue
    gesehen[h] = pfad.name
    jahr = int(pfad.stem.split('_')[2][:4])
    plan = pfad.stem.startswith('Vorsorgeplan')
    lade(pfad, typ='police', jahr=jahr, datum=f'{jahr}-01-01', betrag=None, data=data,
         titel=(f'AXA Vorsorgeplan gueltig ab 01.01.{jahr}' if plan
