...
1. 00_Rechnungen/06_PK/aufbereitet/*.pdf - aus den 105 Handy-Fotos gebaut.
   Die Dokumentgrenzen stehen explizit in sva_dokumente_katalog.csv (Spalte
...
QUELLE = ROOT / '00_Rechnungen' / '04_SVA'

if ERSETZEN:
    alt = sb.table('dokumente').select('id,storage_pfad').eq('user_id', USER) \
            .eq('bereich', 'versicherungen').eq('kategorie', 'ahv').execute().data
...
def lade(pfad, *, typ, jahr, datum, betrag, titel, data=None):
    ...
    sb.table('dokumente').insert({
        ...
        'kategorie': 'ahv', 'jahr': jahr, 'dokument_datum': datum,
        'betrag': float(betrag) if betrag else None, 'referenz': '10.000.969',
...
for r in csv.DictReader(open(HERE / 'sva_dokumente_katalog.csv', encoding='utf-8'), delimiter=';'):
    teile = [r['datum'][:4], 'SVA', r['detail']]
    if r['betrag']:
