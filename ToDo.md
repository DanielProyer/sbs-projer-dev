...
**ENTSCHEID Daniel 29.08.: VOLL-RESET** — alle drei Client-Secrets neu (F/E/G), beide Gmail-Refresh-Tokens neu (OAuth Playground), 5 Passwortmanager-Einträge, Alt-Secrets (C/A/③-alt) erst nach Sammel-Grün löschen. Detaillierte Block-Anleitung: Chat 29.08. DB-Tokens (SBS-Kalender/Kontakte, beide kontakte-google) bleiben bewusst stehen (nicht kompromittiert; Revoke auf Client ① könnte Heinekens Gmail-Grant mitreissen).

- [ ] **Block A — Client ① (1040401919292):** Secret **F** erstellen → PM → SBS `GOOGLE_OAUTH_CLIENT_SECRET`=F (Verif.: Claude-Probe + «Jetzt abgleichen») → Heineken `GMAIL_CLIENT_SECRET`=F → neuer Heineken-Gmail-Refresh-Token via Playground (Redirect-URI temporär eintragen; Konto = Versandkonto, Heineken klärt) → PM → Heineken `GMAIL_REFRESH_TOKEN` (Verif.: Heineken-Notweg). **C nicht löschen bis Sammel-Grün.**
- [ ] **Block B — Client ② (342011182738):** Secret **E** erstellen (A nicht anfassen; kein freier Slot → STOPP) → PM → Heineken `GOOGLE_CLIENT_SECRET`=E (Verif.: Heineken-Probe) → Finanzapp `GOOGLE_CLIENT_SECRET`=E (angekündigt ✓; Verif.: deren Refresh-Test). **A nicht löschen bis beide grün.**
- [ ] **Block C — Client ③ (143053064319):** Consent-Status «In Production» prüfen (Merkposten!) → Secret **G** (2 Slots voll → älteres löschen, kurzer Versand-Blackout unkritisch) → PM → SBS `GMAIL_CLIENT_SECRET`=G → neuer Refresh-Token via Playground (Konto = Rechnungs-Absender) → PM → SBS `GMAIL_REFRESH_TOKEN` (Verif.: Claude-Probe `gmail_refresh`, ohne Mailversand).
- [ ] **Block D — nach Sammel-Grün (Claude gibt GO):** C, A und ③-alt löschen · beide Probe-Functions stilllegen/löschen · Playground-Redirect-URIs wieder entfernen.
- [ ] **Später (auf Zuruf):** Entflechtung «1 Client = 1 Verwendung» (eigene Clients für Heineken-Gmail und Heineken-Kontakte) — erst danach wären auch DB-Token-Erneuerungen gefahrlos.
