...
- [x] ~~**Entscheid Staging-Lauf**~~ **AUSGEFÜHRT 01.09. abends:** `kontakt_uebernahme` hält **exakt 724 Zeilen** (Vollbestand-Lesung mit memberships + gefilterter Upsert — `uebernahme_lesen` je Gruppe hätte die 316 Gruppenlosen nie erreicht). **Vorher-Grösse für den Umzug = 724**, unabhängig gerechnet (768 − 44 Nur-Private), trifft die Erwartung exakt. Volliste liegt strukturiert im Staging (Name/Nummern/Mails/Org/Roh-JSON) — bewusst keine Personendaten-Datei im Repo; lokales Blatt für Daniels Durchsicht bei Bedarf. **Restmessungen da (01.09. abends):** Zielkonto sbs.projer@ = **8 Kontakte** (Duplikat-Prüfung 01.09.: **2 Treffer** — Legna Bar Flims, Parpan - Obertor [dort zusätzlich Quellkonto-Doublette] → **Nachher-Soll 730, bzw. 729 mit vereinigter Obertor-Doublette**; alle Messungen damit KOMPLETT) · Überlappung = **2** (Tino Hassler, «Steven (Privat) - Engadin» — beide auch geschäftlich gelabelt) → Lösch-Schritt umgebaut: erst die 2 im Zielkonto von Privat-Labels befreien, dann per Label löschen. **➡️ Anleitung steht: [kontakte-umzug-anleitung.md](Datenbank/kontakte-umzug-anleitung.md)** — **bereit zur Ausführung durch Daniel** (frühestens 02.09.), alle Vorbedingungen erfüllt.
