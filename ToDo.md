...
- [ ] **Phase 1 — Secrets:** F auf Client ① → SBS `GOOGLE_OAUTH_CLIENT_SECRET` + Heineken `GMAIL_CLIENT_SECRET` · E auf Client ② → Heineken + Finanzapp `GOOGLE_CLIENT_SECRET` · G auf Client ③ → SBS `GMAIL_CLIENT_SECRET`. Je PM-Eintrag; Verifikation mit alten Tokens (Claude-Probe, Heineken-Probe+Testversand, Finanzapp-Test).
- [ ] **Phase 2 — Gmail-Tokens (Konto sbs.projer@):** je Client ① und ③: myaccount-Verbindung entfernen → OAuth Playground (Client-ID + neues Secret, Scope gmail.send) → Token in PM → Heineken bzw. SBS `GMAIL_REFRESH_TOKEN` setzen → Verifikation (Heineken-Testversand / Claude-Probe).
- [ ] **Phase 3 — Kalender/Kontakte-Tokens:** dani.proyer@ myaccount → «SBS Projer» entfernen → SBS-App neu verbinden → Token aus `google_calendar_tokens` in PM. Client ②: je Konto «Finanzapp Kontakte» entfernen → ALLE kontakte-Flows neu (Heineken ×2, Finanzapp; URLs von den Sessions) → Tokens aus deren Tabellen in PM → beide Probes grün.
- [ ] **Phase 4 — nach Sammel-Grün (Claude gibt GO):** C, A, ③-alt löschen · Playground-Redirect-URIs entfernen · Probes abbauen · PM-Vollständigkeit gegen die Landkarte prüfen.
- [ ] **Später (auf Zuruf):** Entflechtung «1 Client = 1 Verwendung» (eigene Clients für Heineken-Gmail und Heineken-Kontakte).
