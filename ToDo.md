...
- ✅ **Edge Function `send-rechnung-mail` abgesichert (v24, 25.09.2026):**
  ...
  mit blossem Anon-Key: 401 (curl); mit Daniels Token: 200, Test-Mail
  angekommen; fremde userId: 403; Pfad-Tricks: 400 (alles am 25.09. geprüft).
...
- **Umsatz-Block fertig** (v0.100.3/v0.100.4): fünf Zeilen rechtsbündig am
...
**1. Beobachten, ob Function v22 hält.** Der Test ist einfach: Nach ein paar Arbeitstagen prüfen, ob noch Mail-Rechnungen auf «offen» stehen bleiben. Die App meldet es seit v0.99.16 selbst (Aufgaben-Glocke «*N* Mail-Rechnungen ohne Versandvermerk»). Kommt die Meldung nie, ist der Fall erledigt.
...
**Korrigiert am 10.09.:** Rechnung 2026-09-1412 (Signina) stand auf «offen», obwohl die Mail am 07.09. um 07:57 raus war — Versandvermerk kam nicht durch, Gefahr Doppelversand. Status auf «gesendet» gesetzt. Abgleich Postausgang ↔ App für 14 Tage: **nur dieser eine Fall**, alle 17 anderen Mail-Rechnungen stimmen.

**⚠️ Die eigentliche Ursache (gefunden 10.09., v0.99.18):** Beide Frühwarnungen für Reinigungen — fehlende Ertragsbuchung UND fehlende Rechnung — filterten mit `.neq('quelle', 'excel_import')`. Das schloss **alles** aus: `quelle` ist bei jeder in der App erfassten Reinigung NULL (211 von 211 seit Juli), und `NULL <> 'x'` ergibt in SQL nicht «wahr», sondern «unbekannt». Beide Suchen lieferten seit ihrer Einführung am 04.09.2026 ausnahmslos eine **leere Liste** — der Nachlauf fand nie etwas, und die Warnung, die das hätte melden sollen, blieb ebenfalls stumm. Filter jetzt im Dart-Code. Neuer Wächter `test/null_filter_waechter_test.dart`. **Lehre:** Zwei Wächter-Tests lasen Kommentare als Code — der neue schlug auf sein eigenes Zitat an, `offenes_datumsfenster_test.dart` schnitt den Query-Block an einem Semikolon im Kommentar ab (falscher Alarm). Beide blenden Kommentare jetzt aus.

**v0.99.16 dagegen gebaut:** (1) Der Nachlauf beim Reinigungsabschluss meldet seine Fehler jetzt — vorher standen sie nur im Debug-Protokoll, weshalb die zwei Buchungen tagelang unbemerkt liegen blieben. (2) Neuer Aufgaben-Detektor **Versandvermerk**: meldet Mail-Rechnungen, die auf «offen» stehen geblieben sind (ab Folgetag, nur `rechnung_mail`).
**Zur Entscheidung (08.09. abends):** App-Analyse mit priorisierten Vorschlägen in `docs/app-analyse-2026-09.md` (als Seite zum Kommentieren: https://claude.ai/code/artifact/064d8e93-fbb1-4c2a-86ac-3c7218717eef) — sechs Befunde (Startseite ist Menü statt Tagesansicht; kein Formular schützt vor Datenverlust beim Zurück-Wischen; Reinigung anlegen = 5 Schritte, kein Direktweg aus Tourenplan/Betrieb; 6 Einsatz-Typen mit 4 Status-Vokabularen; Werkstatt und Büro teilen eine Handy-Startseite; Ballast Events/Bergkundenpauschalen/Anlagen im Hauptmenü) und Vorschläge in drei Stufen A/B/C. Nach Gegenprobe durch 12 Prüfer (3 Korrekturen, 8 Zusatzbefunde). ✅ **A7 erledigt (v0.99.12, 08.09.2026):** Alle 19 Formulare fragen jetzt nach, bevor ungespeicherte Änderungen verloren gehen — `UngespeichertSchutz` + `UngespeichertMixin` in `presentation/widgets/ungespeichert_schutz.dart`, 101 `markiereGeaendert()`-Stellen an Eingaben ausserhalb von FormFields, 20 `geaendertZuruecksetzen()` vor Navigation nach dem Speichern. `test/formular_schutz_waechter_test.dart` hält den Zustand fest, `test/ungespeichert_schutz_test.dart` prüft die Mechanik (6 Tests). In v0.99.13 nachgezogen: die Dialog-Knöpfe stehen untereinander, und `TapKnopf` lässt seinen Text umbrechen — ein 360-px-Test (Pixel 9) zeigte 42 px Überlauf; mit der echten Schrift passte es, mit vergrösserter Systemschrift nicht. ⚠️ **Klicktest Daniel offen:** Formular öffnen → nichts ändern → zurück (darf NICHT fragen); Feld ändern → zurück (muss fragen); «Weiter bearbeiten» und «Verwerfen» je einmal. ✅ **Nutzungszähler läuft seit v0.99.14 (09.09.2026):** Jeder Bildschirmwechsel wird gezählt — Routen-Muster, getrennt nach Handy/PC, Auswertung unter **Einstellungen → Nutzung der App**. Migration 190 (`route_nutzung` + RPC `route_nutzung_zaehlen`). ⏳ **In ein paar Wochen ansehen**, dann sind A6 (Ballast aus dem Hauptmenü) und B2 (Einsätze zusammenführen) entscheidbar statt Geschmackssache. **Nächstes:** A1–A3/A8/A5.
...
- [x] **Dischma `2026-05-0579`, 74.60 — ENTSCHEID DANIEL 08.09.: nichts unternehmen, die Rechnung wird abgeschrieben** («ist schon zu lange her»). Kein Neuversand, kein Anruf. Die Abschreibung selbst ist noch nicht gebucht — wenn es soweit ist, `zahlungsstatus = abgeschrieben` (Wert ist im CHECK erlaubt) plus Ausbuchung des Debitors.

  **Geprüft, wo die gescheiterte Adresse bei uns noch steht:** `betriebe.email` trägt weiterhin `hotel@dischma.ch`. Für Rechnungen und Mahnungen ist das folgenlos — beide lesen `betrieb_rechnungsadressen` (`_kundenEmail()` bzw. `ra?.email` im `MahnwesenService`), dort steht `hotel@hotelconcordia.ch`, und die funktioniert. `betriebe.email` fliesst nur in den Google-Kontakte-Sync. Kein Handlungsbedarf, aber beim nächsten Besuch die gültige Adresse erfragen. Die Heineken-Session führt den Betrieb bewusst weiter mit `hotel@dischma.ch` **plus Vermerk «Adresse fraglich seit 12.06.»** — abgeschrieben ist die Rechnung, nicht die Erkenntnis über die Adresse.

  Zur Vorgeschichte, falls die Frage nochmal kommt: Die Mail ging am 09.06. an `hotel@dischma.ch` und scheiterte endgültig (2 Delays, Failure 12.06.). Ein Neuversand wäre heute an `hotel@hotelconcordia.ch` gegangen (Rechnungsadresse erst am 17.07. angelegt, funktioniert nachweislich) — technisch also möglich gewesen.

**Unabhängig bestätigt (Heineken-Session, 08.09.):** Sie liessen ihren Abruf über 120 Tage laufen — **215 Mails ab dem 20.05., darin kein einziger Rückläufer zu `hotelconcordia.ch`**. Concordia ist damit zugestellt und trotzdem offen: ein **Mahnfall, kein Zustellfall**. Ihre drei Dischma-Meldungen sind ausserdem ein einziger Zustellversuch, kein dreifacher — deckt sich mit meinem Befund.

- [ ] **Concordia `2026-05-0580`, 74.60, seit 08.05. offen — Mahnfall.** Die Rechnung ist nachweislich zugestellt, bezahlt wurde sie nicht. Gehört in den normalen Mahnlauf, nicht in die Zustell-Nacharbeit.
...
- [ ] **`versendet_am` ist ein `date`, kein Zeitstempel — ein Neuversand am selben Tag hinterlässt keine Spur.** Aufgefallen beim Alpenblick-Fall: Bounce 10:13, Neuversand 11:25, beide am 03.09. — `versendet_am` bleibt in beiden Fällen `2026-09-03`. Auch der Code schreibt bewusst nur das Datum (`toIso8601String().split('T').first`, an fünf Stellen plus der Edge Function). Der Beleg für den Neuversand liess sich **nur im Postfach** finden, nicht in unseren Daten. Die Heineken-Session löst das bei sich über «versendet nach dem Rückläufer» — das greift hier nicht, solange die Auflösung ein Tag ist. Umstellung auf `timestamptz` wäre eine kleine Migration, aber alle Lesestellen müssten mit.
...
- **camt-Import**: Steuer-Vorschläge (Vorlage auf 8900/2208/2202) zeigen Dropdowns Steuerjahr/Steuerart; Kontierung automatisch 2208 (solange Rückstellung des Jahres offen) / 8900 / 2202, Restsaldo wird innerhalb eines Imports fortgeschrieben.
...
- [x] **⚠️ Zwei Nebenbefunde aus dem Review (beide behoben, `5e085c2`):** (1) **Die Suche nach Reinigungen ohne Rechnung schnitt seit Kurzem stumm ab** — sie lud «alle abgeschlossenen Reinigungen ab 01.12.2025» in einem Request, PostgREST liefert höchstens 1000, und aktuell liegen 1022 im Fenster. Die 22 ältesten (01.–04.12.2025) waren für die Warnung unsichtbar, und der Abstand wächst mit jeder Reinigung. Jetzt seitenweise, neuer Wächtertest `offenes_datumsfenster_test.dart`. (2) Der Nachlauf hätte **rückwirkend ins bilanzierte 2025 buchen können**, wenn bei einem Betrieb die Rechnungsstellung umgestellt wird (856 von 1022 Reinigungen haben keine eigene Zahlungsart und hängen am Betrieb; 25 unverbuchte Heineken-Fälle über CHF 3'522 wären dann buchungspflichtig geworden). Jetzt Untergrenze = 1. Januar des ersten Jahres ohne Abschlussbuchung; ältere Fälle werden nur gemeldet, und der Knopf zeigt die Liste vor dem Buchen.
- [x] **Fix gebaut 04.09. (Commits `8d950ed` + `5e085c2`, v0.97.0 live, TDD, 1293 Tests):** (1) `BuchungNachholService` findet abgeschlossene Reinigungen ohne Ertragsbuchung und bucht sie nach — idempotent, Blockabfrage gegen die 1000-Zeilen-Grenze; läuft **automatisch nach jedem erfolgreichen Abschluss** (Fenster 14 Tage, nur wenn die eigene Buchung eben durchlief = Verbindung nachweislich ok) und **auf Knopfdruck** in der Warnung der Forderungen-Liste. (2) 15. Regel «Reinigungen ohne Ertragsbuchung» in der Abschlussprüfung (rot, Tap → Forderungen). (3) `brauchtErtragsbuchung()` ist neu die eine geteilte Entscheidung für Buchen und Suchen — vorher hätte die Suche eine driftende Kopie gebraucht. **Nebenbefund:** `warnungsGrund` meldet `jahresrechnung` nie, obwohl sie gebucht wird — die neue Suche deckt das ab.
...
0. **2026-Folgen des Abschlusses:** ~~Belastung 05.05.2026 prüfen~~ **erledigt 02.09.** (Bund 15.04. 2'405.50 + Kanton 05.05. 2'748.00 = provisorische 2025 → beide auf 2208 umgebucht, `JA2025_D_U1/U2`; Rückstellung 4'000 damit um 1'153.50 überzogen — die Abschlussprüfung zeigt das als 2208 im Soll, bei der definitiven Veranlagung 2025 gegen 8900 ausgleichen); Q3/2026-MWST: Ziff. 235 159.90; ~20 Tresen-Rechnungen Dez 2025 (per Excel-Delta bezahlt, Status noch «offen») nachziehen — die Regel «Rechnungen offen mit gebuchter Zahlung» in der Abschlussprüfung listet sie.
...
4. **Entflechtung — Vorhaben ① KOMPLETT 02.09.** (Plan: `Datenbank/oauth-entflechtung-plan.md`): Kontakte-Umzug ausgeführt und verifiziert — **sbs.projer@ = 723 Geschäftskontakte, dani.proyer@ = 46 Private (alle Label «Privat»)**; Ablauf + Lehren in `Datenbank/kontakte-umzug-anleitung.md`. **Als Nächstes: Vorhaben ②** (GCP-Projekt «Heineken Plattform» unter sbs.projer@, 2 Clients, Heineken stellt um — Achtung: Client-IDs ändern sich!), danach ③ Kalender-Umzug. **Absprache mit den Sessions Heineken + Finanzapp ist Pflicht** (Memory: projektuebergreifend-absprechen). Separat, blockiert nichts: Daniels Kontakte-Komplett-Durchsicht in sbs.projer@ (33 Namensdoppel, 75 geteilte Nummern, Merkposten-Liste in der Anleitung)
...
### Arbeitsstand (Zahlen-Zusammenstellung, erledigt)
...
**Die wichtigste Lehre — dreimal am selben Tag bestätigt:** Eine plausible Ableitung ersetzt keine Messung. Der ältere Schlüssel war bei Client ① ein Waisenkind und bei Client ③ der aktive; die unerklärte Kontakte-Freigabe gehörte nicht dem vermuteten Konsumenten. Und ein grüner Refresh beweist nur, dass *irgendein* gültiges Token wirkt — dass es das **neue** ist, zeigt erst der Schreibzeitstempel der Token-Zeile.

### Vorgeschichte (Befundlage vom 29.08. vormittags)

**Befund (Screenshot Daniel 29.08.):** «Jetzt abgleichen» wirft `502 — The provided client secret is invalid`. Das ist die wörtliche Google-Antwort beim Token-Refresh in [google-calendar-sync](supabase/functions/) (`GOOGLE_OAUTH_CLIENT_ID`/`GOOGLE_OAUTH_CLIENT_SECRET` aus den Supabase-Secrets): **Das Secret in Supabase passt nicht mehr zum Google-Client.** Letzter erfolgreicher Abgleich 27.08. 00:58.

**Ursache GEKLÄRT (Daniel, 29.08.):** Am 27.08. spätabends wurde für die Heineken-Kundenrechnung (E3.1) ein **neues Client-Secret erstellt und wegen des 2-Secrets-Limits ein altes GELÖSCHT** — das gelöschte war das, das SBS Projer noch nutzt. **Kein Google-Fehler, sondern geteilte Secrets über Projekte hinweg ohne Konsumenten-Liste.** Projektübergreifende Bestandsaufnahme + Governance-Vorschlag: siehe Chat 29.08.; Abstimmungs-Nachrichten an die Sessions Heineken und Finanzapp sind raus. Heinekens `rechnung-versand` läuft bereits auf einem EIGENEN neuen Google-Cloud-Projekt (1040401919292) — das ist das Vorbild («1 Client = 1 Verwendung»). ⚠️ Heinekens `kontakte-google` nutzt den **Finanzapp**-Client mit (dokumentierte Querabhängigkeit).
...
**Heineken-Session bestätigt per Live-Probe (29.08.):** Auch der **Finanzapp-Client** ist kaputt (401 invalid_client bei beiden Konten) → Heinekens `kontakte-google` steht ebenfalls; fiel mangels Cron nur nicht auf. Heinekens `rechnung-versand` ist sauber (eigenes Projekt, Code geprüft, Testversand nach der Rotation bestanden). Restunsicherheit: SBS-Kalender sollte laut Bauplan (10.07.) ein EIGENER Client «SBS Projer Web» sein — entweder wurde damals doch der Finanzapp-Client wiederverwendet (dann heilt EIN Secret alles), oder es sind zwei Clients betroffen. Auflösung empirisch beim Fix.

**Endgültiges Bild (SBS-Probe + Heineken-Probe + Finanzapp-Probe, 29.08.): Es sind DREI Clients.** ① GCP **1040401919292** («SBS Projer», Juli): SBS-Kalender (`1040401919292-cev05…`) — HIER fand die Rotation vom 27.08. statt, SBS hält das gelöschte Secret; Heinekens Gmail-Versand nutzt das neue Secret C aus demselben Projekt. ② GCP **342011182738** («Finanzapp Kontakte»): Finanzapp läuft (Secret A, nur in deren Supabase), Heinekens kontakte-google-Kopie ist tot (separater, älterer Bruch). ③ GCP **143053064319**: SBS-Gmail-Versand (`143053064319-3ftn…`) — **refresh_ok, war NIE betroffen** (per Probe belegt, ohne Mailversand). Detaillierte Schritt-für-Schritt-Anleitung: Chat 29.08.

**ENTSCHEID Daniel 29.08. (erweitert): KOMPLETT-RESET** — alle drei Client-Secrets neu (F/E/G) UND alle Tokens neu (Gmail via OAuth Playground, Kalender/Kontakte via Verbindung-entfernen + Neu-Autorisieren), ~9 Passwortmanager-Einträge. Kernregel der Reihenfolge: **erst Secrets setzen und mit ALTEN Tokens verifizieren, dann je Google-Konto App-Verbindung entfernen und SOFORT neu autorisieren** (Entfernen killt alle Tokens des Kontos auf dem Client — bei den geteilten Clients mehrere Apps zugleich). Alt-Secrets (C/A/③-alt) erst nach Sammel-Grün löschen. Detail-Anleitung: [Datenbank/oauth-komplett-reset-anleitung.md](Datenbank/oauth-komplett-reset-anleitung.md); alle drei Projekte einverstanden, Details bestätigt.

- [x] ~~**Phase 1 — Secrets**~~ **GRÜN 29.08. ~14:50.** F/E/G gesetzt, je PM-Eintrag, alle fünf Wege verifiziert: SBS-Kalender `refresh_ok` + echter Abgleich · SBS-Gmail `refresh_ok` · Heineken-Gmail ok/200 + echter ist_test-Versand (Message-ID `1a04d83faac9364a`, Doppel-Aufruf verweigert) · Heineken-Kontakte beide Konten ok/200 · Finanzapp `refresh_ok`. **Bestätigt:** alte Tokens überleben den Secret-Wechsel überall — Phase 3 ist reine Token-Rotation, keine Reparatur. **Vier Befunde:** (a) Client ① hatte KEINEN freien Platz (2 Schlüssel 15.08./27.08.); der gelöschte 15.08. war per Messung ein Waisenkind, Heinekens ist der 27.08. (b) Client ③ hatte ebenfalls 2 (20.04./27.04.) — dort war der gelöschte ältere der AKTIVE (Probe direkt danach: 401 «client secret is invalid»), geheilt durch G nach <2 min. ⇒ **«lösch den älteren» begrenzt nur den Schaden, es sagt nichts über ungenutzt — nur die Messung unmittelbar danach ist verlässlich.** (c) Projekt `143053064319` gehört **sbs.projer@**, nicht dani.proyer@; der Projektwähler findet die **Projektnummer nicht** (nur Name/Projekt-ID) → URL `?project=<nr>&authuser=<konto>`. (d) Feld-Merkregel: «The OAuth client was not found» ⇒ Wert in der falschen Zeile (ID statt Secret); «The provided client secret is invalid» ⇒ richtige Zeile, falscher Wert.
- [x] ~~**Phase 2 — Gmail-Tokens**~~ **GRÜN 29.08. ~15:10.** Beide Tokens neu über den OAuth Playground, je PM-Eintrag. Heineken: Probe ok/200 (vorher `invalid_grant`) + echter ist_test-Versand TEST-2026-0006, Message-ID `1a04da026e260dee`, Doppel-Aufruf verweigert. SBS: Probe `refresh_ok`. **Befund:** Client ① und ③ tragen **denselben** Zustimmungsbildschirm-Namen «SBS Projer» — unter sbs.projer@ zwei gleichnamige Einträge, weder am Namen noch am Scope (beide `gmail.send`) unterscheidbar → beide entfernt, beide Tokens neu geholt. Einer der beiden trug zusätzlich eine **Kontakte**-Freigabe ohne dokumentierten Konsumenten (vermutlich Karteileiche einer versehentlichen Verbindung der SBS-App mit sbs.projer@ statt dani.proyer@); Kontrollmessung bestätigte: Heinekens Kontakte hängen an Client ②, unberührt. **Dritter Fehlerfall gelernt:** `invalid_grant` (400) = Secret stimmt, Token widerrufen — nicht mit `invalid_client`/«secret is invalid» verwechseln. Nebenbei: Heineken setzte bei TEST-2026-0006 den Zustellweg «tresen» → «mail» (sonst verweigert `rechnung-versand` den Versand) und liess ihn bewusst so.
- [x] ~~**Phase 3 — Kalender/Kontakte-Tokens**~~ **GRÜN 29.08. ~15:45** (Finanzapp-Bestätigung stand bei Redaktion noch aus). Teil F: «SBS Projer» unter dani.proyer@ entfernt, App neu verbunden, Kalender-Abgleich + Kontakte-Sync fehlerfrei, Probe `refresh_ok` mit neuem DB-Token. Teil G: alle drei Freigaben neu; Heineken-Probe beide Konten ok/200, **Schreibzeitstempel 15:28:13 / 15:30:07 als Beleg, dass wirklich die neuen Tokens wirken** (das Grün allein hätte nichts bewiesen — die alten waren auch gültig). **Stolperstein `GOOGLE_OAUTH_STATE`:** Secret-Werte lassen sich im Supabase-Dashboard hier nicht aufdecken und die alten Werte waren nirgends dokumentiert (Heineken-Repo durchsucht: steht weder in Anleitung noch Code) → beide STATEs **neu gesetzt** (48 Zeichen alphanumerisch, kein URL-Encoding nötig) und in den PM gelegt; Finanzapp gab Weg B ausdrücklich frei nach Code-Prüfung (einziger Konsument, live aus Env). **Grundsatz bestätigt:** Secret-Werte gehen nicht durch den Session-Nachrichtenkanal. **Fremdbefund:** Heinekens `kontakte-google` sendet die Erfolgsseite ohne `Content-Type: text/html; charset=utf-8` → rohe `<h2>`-Tags und «fÃ¼r», sieht wie ein Fehlschlag aus; liegt auf ihrer Liste.
- [x] ~~**Phase 4 — Aufräumen**~~ **GRÜN 29.08. ~15:55.** C (Client ①), A (Client ②) und das 27.04.-Waisenkind (Client ③) gelöscht, beide Playground-Redirect-URIs entfernt. **Jede Löschung einzeln mit Messung danach** — alle drei grün: Heineken nach C (Gmail + beide Kontakte ok/200), Finanzapp nach A (`refresh_ok` mit dem Token von 15:37), SBS-Schlussprobe nach ③ (`kalender_refresh`/`gmail_refresh` beide `refresh_ok`). Endstand: je Client genau EIN Secret, alle drei im Passwortmanager mit Konsumenten-Liste. Alle drei Probe-Functions auf 410-Rumpf zurückgebaut (SBS-Probe von mir, verifiziert: liefert 410 auch mit altem Probe-Schlüssel).
- [x] ~~**Probe-Functions endgültig löschen**~~ **ERLEDIGT 29.08.** Alle drei `google-secret-probe` gelöscht (SBS `pltbaqqwpnmdajwgnhpd`, Heineken `dtdcohqjihoiqlaekeiz`, Finanzapp `botsyitkcxxkwpslsssq`), verifiziert: kein Treffer mehr in der Function-Liste, Endpunkt antwortet 404. Ging per CLI (`npx supabase functions delete google-secret-probe --project-ref <ref>`) — die CLI ist auf allen drei Projekten angemeldet, das Dashboard war nicht nötig. **Merken:** Der Supabase-MCP-Server kann Functions nur anlegen und lesen, nicht löschen; dafür ist die CLI der Weg.
## 🟡 GEPLANT ab KW 36: Entflechtung «1 Client = 1 Verwendung» + Trennung geschäftlich/privat

**Entwurf freigegeben 29.08. (Reihenfolge A), Umsetzung nächste Woche.** Vollständig: [oauth-entflechtung-plan.md](Datenbank/oauth-entflechtung-plan.md).

**Zwei Ziele:** (a) jeder OAuth-Client trägt genau eine Verwendung — heute tragen ① und ② je zwei, und genau diese Bauart hat den Ausfall vom 27.08. verursacht; (b) klare Trennung **geschäftlich sbs.projer@ / privat dani.proyer@**, auf beiden Achsen: Projekt-Eigentümerschaft **und** welche Konten die Apps lesen. **Entscheide:** ein Heineken-Projekt mit zwei Clients (Scopes unterscheiden sie) · Finanzapp gilt als **privat** und bleibt unter dani.proyer@ · Geschäftskalender wird an dani.proyer@ **freigegeben** statt zweites Konto aufs Handy · Eigentümerschaft von Projekt `1040401919292` per **IAM umhängen** statt neu bauen (spart den Neubau von Client ①).

- [x] ~~**① Kontakte-Umzug**~~ **KOMPLETT 02.09., Abschlussmessung GRÜN** — sbs.projer@ 723 (verifiziert gegen Staging, kein Privater im Ziel), dani.proyer@ 46 (alle «Privat»; Tino Hassler + «Steven (Privat) - Engadin» bewusst privat geblieben, Steven-Nummern getrennt). Ablauf, Zahlen und Lehren (u. a. Seiten-Auswahl-Falle bei «Alle auswählen»): `Datenbank/kontakte-umzug-anleitung.md`. *(Ursprünglicher Planungstext:)* geschäftliche Kontakte dani.proyer@ → sbs.projer@. `konto=dani` ist Heinekens **Lesequelle**, nicht bloss ein Regelbruch — Abklemmen erst, wenn die Kontakte im Geschäftskonto liegen. 🔴 **Es sind FÜNF geschäftliche Gruppen, nicht eine** (Vollmessung 29.08. abends, rein lesend): **SBS App 307** · Geschäftlich 88 · SBS Kunden 25 · SBS Heineken 19 · Gampel 2026 1 · SBS Event + SIM ohne Zahl → **~440 Mitgliedschaften**. Privat daneben 91 (Privat 45, Freunde 15, Bekannte 14, Familie 11, Juma 4, Mexiko 1, Studium 1). **Die 731 ist geklärt:** nie die Gruppengrösse, sondern die Gesamtzahl der Kontakte im Konto (aktuell 768). Wer nur «Geschäftlich» umzieht, lässt die 307 aus «SBS App» liegen — und der Umzug meldet trotzdem Erfolg.
- [x] ~~**🔵 Vor dem Umzug klären (Daniel)**~~ **ALLE DREI ENTSCHEIDE GEFÄLLT 01.09.** (Messungen: Heineken-Session, Vollbestand 768 via People-API mit memberships, rein lesend): (1) **Alle 5 geschäftlichen Gruppen ziehen um** (SBS App 307, Geschäftlich 88, SBS Kunden 25, SBS Heineken 19, Gampel 2026 1) = **408 eindeutige Personen** (440 Mitgliedschaften, 32 Mehrfachnennungen). «SBS Event», «SIM» und «Importiert am 15.12.24» sind **gemessen leer** — kein Entscheid nötig, die memberCount-Annahme war korrekt. (2) Personenzahl gemessen: s. o. (3) **Gruppenlose sind 316, nicht ~237 — und zu ~98 % geschäftlich** (309× Namenskonvention «Ort - Betrieb (Person Du)» inkl. 4× U+2010-Bindestrich, 17× «Heineken - Vorname», 6× Org-Feld, 3× Firmen-Mail; unklar nur Combox, 1 Namenloser, Maria Roth, Forstamt Arosa, Sonnenbräu Monteur). **Entscheid Daniel: alle 316 mitnehmen.** Kontenbild: 768 = 408 (Geschäftsgruppen) + 44 (nur Privatgruppen) + 316 (gruppenlos) → **Umzugsmenge = 724 Personen**, nicht ~440.
- [ ] **🆕 Kontakte-Durchsicht nach dem Umzug (Daniel):** alle umgezogenen Kontakte einmal durchgehen — behalten/löschen entscheiden, bei Bedarf umbenennen/korrigieren und jeder einer Kategorie/Gruppe zuweisen (Wunsch Daniel 01.09., gehört zu Vorhaben ①, blockiert ②/③ nicht).
- [x] ~~**Entscheid Staging-Lauf**~~ **AUSGEFÜHRT 01.09. abends:** `kontakt_uebernahme` hält **exakt 724 Zeilen** (Vollbestand-Lesung mit memberships + gefilterter Upsert — `uebernahme_lesen` je Gruppe hätte die 316 Gruppenlosen nie erreicht). **Vorher-Grösse für den Umzug = 724**, unabhängig gerechnet (768 − 44 Nur-Private), trifft die Erwartung exakt. Volliste liegt strukturiert im Staging (Name/Nummern/Mails/Org/Roh-JSON) — bewusst keine Personendaten-Datei im Repo; lokales Blatt für Daniels Durchsicht bei Bedarf. **Restmessungen da (01.09. abends):** Zielkonto sbs.projer@ = **8 Kontakte** (Duplikat-Prüfung 01.09.: 2 Treffer — **von Daniel spätabends überholt:** Zielkonto auf **3** reduziert [Heineken Urs, Naella. = Heineken-Eventmanagerin, Your = Daniels Nummer], Alt-Legna/Alt-Obertor/Ervin JANZ/Rolf Petschen gelöscht, Reto Baumann → dani.proyer@, Privat-Labels laut Daniel auf «Privat» konsolidiert → **Duplikate Ziel∩724 = 0, Nachher-Soll voraussichtlich 726**; verbindlich erst nach Frisch-Messung = neuer Schritt 0 der Anleitung, unmittelbar vor dem Umzug) · Überlappung = **2** (Tino Hassler, «Steven (Privat) - Engadin» — beide auch geschäftlich gelabelt) → Lösch-Schritt umgebaut: erst die 2 im Zielkonto von Privat-Labels befreien, dann per Label löschen. **➡️ Anleitung steht: [kontakte-umzug-anleitung.md](Datenbank/kontakte-umzug-anleitung.md)** — **bereit zur Ausführung durch Daniel** (frühestens 02.09.), alle Vorbedingungen erfüllt.
- [ ] **② OAuth-Entflechtung:** Projekt «Heineken Plattform» unter sbs.projer@, zwei Clients + Secrets + PM-Einträge, Heineken stellt um (⚠️ diesmal ändern sich auch die Client-**IDs**, nicht nur die Secrets), neues Gmail-Token + Kontakte-Flows, dann Heinekens Verwendungen aus ① und ② entfernen, zuletzt `1040401919292` per IAM auf sbs.projer@ umhängen.
- [ ] **③ Kalender-Umzug:** SBS-App auf sbs.projer@ neu verbinden, Kalender **aus der DB neu aufbauen** (Termine müssen nicht einzeln umziehen — die App erzeugt alle sechs Typen selbst), Freigabe an dani.proyer@ fürs Handy, Alt-Termine privat aufräumen, Heinekens `konto=dani` abklemmen.
...
«Nur Aufträge (Störungen, Montagen usw.) und Eröffnungs-/Endreinigungen im Kalender, wie bisher» — und genau das läuft bereits (Kartierung 13.08.): Push für pikett/event/termin (= Eröffnungs-/Endreinigung)/einsatz (= Störung+Montage beim Einplanen)/betrieb_reinigung/betrieb_manuell, täglicher Auto-Reconcile, OAuth serverseitig. **Kein Bau nötig; der Memory-Merkposten «Kalender-Anbindung offen» ist damit vom Tisch.** Optionale Kleinigkeiten, NUR auf Zuruf: `location` (Betriebsadresse) an Einsatz-Termine setzen → Navigation direkt ab Kalender; tote `termine.erinnerung_*`-Spalten ausbauen (Edge Function nutzt Hardcode-Reminders).
...
- [x] ~~**Daniel:** In der Google Cloud Console prüfen, ob die OAuth-App auf «In Produktion» steht~~ **BESTÄTIGT 29.08.:** Zustimmungsbildschirm von Projekt `143053064319` (Client ③, SBS-Rechnungsversand) steht auf **«In Produktion»**. Damit kein 7-Tage-Ablauf der Refresh-Tokens. Zugang nur als **sbs.projer@** (Projekt gehört diesem Konto).
...
**Entscheid Daniel:** Variante 1 — **alles bleibt wie es ist.** «In SBS Projer DEV wird aktuell nur die Webapp benutzt, für die neue App bleibt die Offline/Isar-Schiene wichtig, weil dort dann nur noch die Android-App genutzt wird.»

**Damit ist der native Pfad kein toter Code, sondern die Vorlage.** Kein Rückbau, kein Wachstums-Stopp; die 13-Schritte-Checkliste «Neue Entity hinzufügen» in `CLAUDE.md` bleibt vollständig gültig.

**Arbeitsregel daraus:**
- **Bugs im nativen Pfad weiterhin fixen** — sie wandern sonst 1:1 in die Android-App, wo sie erst wirklich beissen (echte lokale Kopien, echtes Offline). Die beiden Fixes aus v0.91.0 sind genau dieser Fall und damit **nicht** umsonst gewesen.
- Beim Melden sauber trennen: «wirkt im Alltag hier nicht — Wert liegt in der Vorlage für die neue App».
...
**v0.82.0 (Feedback Daniel zum v0.81-Umbau):** Genauigkeit wieder als Klartext neben dem Standort-Icon («genau»/«mittel»/«ungefähr»/«kein Standort», farbig) — Icon-Farbe allein war zu subtil. **Tel/WhatsApp-Direktwahl** (Ein-Tipp) rechts in der Stand-Zeile, sobald ein Kontakt mit Nummer verbunden ist; Optik wie im Kontakte-Tab.
...
- [x] ~~Saisonstarts~~ **ERLEDIGT 13.08. (Recherche 2 Agenten + Freigabe Daniel):** **Steakhouse Ochsen ist GANZJAHRESBETRIEB** (Entscheid Daniel; online täglich 12–23) → Saisonbetrieb-Kennzeichen entfernt, erscheint wieder im Tourenplan (fällig seit 17.01., 205 T). **Winter-Starts 2026/27 gesetzt:** Fuxägufer + Clavadeleralp 27.11. (Jakobshorn-Start) · Piz Piz + Dischma 04.12. · Milez 12.12. (offizieller Bahnstart) · Alp Lavoz 12.12. · Frosch Sportclub 19.12. (bestätigt, erster Anreisetermin) · Nagens 19.12. (LAAX-Hauptsaison). Alte Winter-ENDEN (Frühjahr 2026) bewusst belassen — die Fenster-Logik (Start>Ende = Jahreswechsel) verkraftet das; Enden 2027 im Frühjahr pflegen.
...
- [x] ~~Prüfpunkt: Sammelzahlung 26.06. Weisse Arena~~ **AUFGEKLÄRT + KORRIGIERT 08.08. (v0.73.3):** War gar nicht Weisse Arena, sondern **LHG Lifestyle Hotels (Me and All Hotel Flims)**. Der Bank-Vermerk nannte ZWEI Rechnungsnummern (253.00 = 2 × 126.50), die zweite mitten in der Nummer umbrochen («2026-04-047 5» = 2026-04-0475) — der Parser nahm nur die erste. Korrektur: Mehrzahlungs-Buchung (8000) gelöscht, Zahlungseingang für 2026-04-0475 gebucht (26.06., camt-Schlüssel), Rechnung bezahlt; Bank 1020 unverändert exakt. **Code-Härtung v0.73.3:** `parseVermerk` extrahiert jetzt ALLE Rechnungsnummern und repariert Bank-Zeilenumbrüche mitten in der Nummer; neue Abgleich-Stufe 1.5 (alle genannten Nummern offen + Summe = Zahlbetrag → Auto-Vorschlag «Rechnungsnummern im Vermerk»). +7 Tests.
...
4. **Zahlernamen aus Zahlungshistorie gelernt** (Auftrag Daniel: letzte 4 Zahlungen, alle gleich → auto): 1'606 Gutschriften ab 2024 aus der Voll-camt gegen bezahlte Rechnungen gematcht (nur eindeutige Datum+Betrag-Paare). **57 Aliase automatisch gesetzt**; 53 Nachfragen → Listen in `docs/camt-zahlernamen-lernen-2026-08-07.md`. Arbeitstabellen/Rollback: Schema `tmp_zahlername`.
5. **Internet-Recherche zu den 53 offenen Fällen (3 Agenten, Zefix/Impressen/News): 49 aufgeklärt → 52 weitere Aliase auf 42 Betriebe** (Belege im Doc). Korrekturen: Alpenblick-Arosa-Alias entfernt (gehört Weggis), Lenzerhorn-Alias entfernt (Hotel betreibt auch Spescha → Zwei-Betriebe-Zahler). Bewusst OHNE Alias: HR Seehof (Seehof+Chesa), M&M Gastro (Giodavin+Ochsen), Allegra Vinum (Franziskaner+Fondue Beizli), Traube Senti (Alt-Wirt Don Camillo). **AlpinTrend-Gruppe (Bergamin) erklärt 5 Zahler.** **Restfragen von Daniel beantwortet → ALLE 53 FÄLLE ABGESCHLOSSEN** (BARacca←Pagels per Moneyhouse-Beleg, Weissfluhgipfel←racine, Weissfluhjoch←surselva hospitality ag — Joch Sommer 2026 zu, neuer Pächter ab Winter). **Bestand: 123 Betriebe / 135 Aliase** (vor heute 25). Merkposten: Don-Camillo-Alias (Blazevic) erst bei erster Zahlung; Löwen Sins seit ~04/2026 neuer Betreiber (Peterhans).
...
- [ ] **(9) Klärungen klein**: 14 Ertrags-Belegvarianten Excel↔App (1'510.30, Rundung/Datum/Preis — Liste beim Audit); `abgerechnet`-Flags 42 Pauschalen + 22 Heineken-Reinigungen Dez–Mai auf true (kosmetisch, verifiziert enthalten).
- [x] **Kassenbestand GEKLÄRT (08.08., Tiefenprüfung):** DB 1000 = **11'084.28** ist journalgetreu und rappengenau abgestimmt. Daniels Zweifel kam von der **Excel-Bilanz (2'041.24 per 31.12.2025) — die ist FALSCH (zu tief)**: Das Excel-Hauptbuch hat die Journal-Kassenzugänge **29.01.–März 2025 nie übernommen** (Übertragungslücke, 59 Zugänge = 5'317.85, Februar komplett; dazu 774.65 Abgänge) — gleicher Fehlertyp wie der MwSt-Befund (Journal↔Hauptbuch inkonsistent). Abstimmung: Bilanz 2'041.24 + Lücke netto 4'554.40 + Seeblick Sufers 23.12. (nur App, 74.60) = DB 6'670.24 per Ende 2025 ✓; Bank 1020 Excel↔DB exakt gleich (12'202.73) ✓; Live-Ära ab Dez 2025 duplikatfrei verifiziert (kein Kasse+Bank-Doppel, beide Automaten-Einzahlungen erfasst). **Offen: Kassensturz** — Soll 11'084.28; Differenz als Privatbezug 2260 an 1000 buchen. **Zwischenstand 08.08.:** Daniel hat gezählt — knapp 5'000 in Scheinen + grosse Münzkiste («könnte hinkommen»); Münz wird am GKB-Automaten einbezahlt → die Einzahlung liefert via camt den exakten Münzbetrag, danach definitiver Abgleich (Scheine + Münz + Portemonnaie-Rest vs. Buchwert). Merkposten: Kasse war 21.05.2024 rechnerisch −2'042 (Alt-Ära nie Bestandes-geführt); Scanner-Regel: Kässeli-Geld → «bar», nur echtes Privatgeld → «privat».
