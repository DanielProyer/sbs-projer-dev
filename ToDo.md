...
- [ ] **Daniel — Google Cloud Console** (*APIs & Dienste → Anmeldedaten*): **Alle OAuth-Clients durchgehen.** Bei jedem Client mit Secret-Erstelldatum 27.08.: den neuen Secret-Wert kopieren (falls nicht mehr sichtbar: im freien Slot frisch erstellen). Dabei notieren, welche Clients es überhaupt gibt (SBS Projer Web? Finanzapp? Gmail-Versand?) → das wird die Konsumenten-Liste.
- [ ] **Daniel — drei Supabase-Projekte versorgen:** SBS `GOOGLE_OAUTH_CLIENT_SECRET` (pltbaqqwpnmdajwgnhpd) · Heineken `GOOGLE_CLIENT_SECRET` (dtdcohqjihoiqlaekeiz) · Finanzapp `GOOGLE_CLIENT_SECRET` (botsyitkcxxkwpslsssq). Danach in der SBS-App «Jetzt abgleichen» → Claude verifiziert Logs; Heineken prüft über die stehende `google-secret-probe`.
