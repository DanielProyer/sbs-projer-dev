...
- [ ] **Phase 1 (Daniel):** GCP 1040401919292 → Client `…cev05…` → neues **Secret D** erstellen (freier Slot vorhanden; **C nicht löschen** — Heineken-Versand!) → Wert sofort in den Passwortmanager → SBS-Supabase `GOOGLE_OAUTH_CLIENT_SECRET` = D → «eingetragen» melden → Claude verifiziert per Probe + App-Abgleich.
- [ ] **Phase 2 (Daniel):** GCP 342011182738 → Client öffnen → **Secret A nicht anfassen**; freier Slot → neues **Secret E**; KEIN freier Slot → STOPP und melden (nicht raten, welches A ist) → Passwortmanager → Heineken-Supabase `GOOGLE_CLIENT_SECRET` = E → «eingetragen» melden → Heineken-Session verifiziert (Grün: beide Konten 200).
- [ ] **Phase 3 (Daniel):** Drei Passwortmanager-Einträge nach Schema (Client-ID als Nutzername, Secret als Passwort, Konsumenten-Liste als Notiz) — Details im Chat. Bestätigt: Stand 29.08. liegt dort NICHTS.
- [ ] **Phase 4 (nach Grün):** Probes abbauen (SBS + Heineken je 410-Rumpf, endgültig löschen im Dashboard bei Gelegenheit).
- [ ] **Phase 5 (auf Zuruf, angekündigt):** Vereinheitlichung Client ①: Heineken `GMAIL_CLIENT_SECRET` auf D → Testversand → C löschen. Client ②: Finanzapp auf E (deren Bedingung: Ankündigung + eigener Refresh-Test) → erst nach Grün A löschen. Härtung Client ③: geplante Rotation, um das Gmail-Secret erstmals zu sichern (Refresh-Token bleibt ohne Backup — nur per Neu-OAuth ersetzbar); dabei OAuth-App-Status «In Production» prüfen (alter Merkposten).
