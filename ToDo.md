...
- [ ] **① Kontakte-Umzug** (Voraussetzung für den ganzen Umbau, nicht nur ③): geschäftliche Kontakte dani.proyer@ → sbs.projer@. **Erkennung geklärt** (Heineken-Befund 29.08.): Google-Gruppe **«Geschäftlich», 731 Kontakte** — die Gruppe ist die Definition, keine Heuristik nötig. ⚠️ Diese 731 sind bei Heineken **noch nie angekommen** (Staging 0, `google_resource_name` 0, `kontakt_person` gesamt 117) — und `konto=dani` ist Heinekens **Lesequelle**, nicht bloss ein Regelbruch: Abklemmen erst, wenn die 731 im Geschäftskonto liegen.
- [ ] **🔵 ENTSCHEID DANIEL (Montag zuerst):** Heineken schlägt vor, die 731 **vor** dem Umzug einmal per `uebernahme_lesen` ins Staging zu lesen — rein lesend, bei Google ändert sich nichts. Gründe: der Umzug wird prüfbar (731 vorher/nachher statt Annahme), und es gibt **sonst keine zweite Kopie** (die Alt-App kennt diese Kontakte nicht). Ich habe **kein Grün gegeben**: Der Lauf schreibt 731 Kontakte aus dem privaten Konto in die Heineken-DB — dieser Datenfluss ist Daniels Entscheidung. Kein Zeitdruck, `konto=dani` bleibt bis ② bestehen.
