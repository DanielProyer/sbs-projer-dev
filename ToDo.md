...
**Finanzapp-Probe (29.08., eigene Session): deren Secret läuft NOCH (HTTP 200)** — die beiden Supabases hielten unterschiedliche Secret-Werte desselben Clients; gelöscht wurde nur das Heineken-/SBS-Secret. **Finanzapp braucht KEINEN Eintrag.** Auf dem Client liegen jetzt: Secret A (alt, gültig — Finanzapp; **nicht löschen!**) + Secret C (neu vom 27.08.).

- [ ] **Daniel — Google Cloud Console** (*APIs & Dienste → Anmeldedaten*, betroffener Client): Wert von **Secret C** (27.08.) noch vorhanden? **Ja** → verwenden. **Nein** (Google zeigt ihn nur bei Erstellung) → C löschen (nutzt niemand), **Secret D** erstellen, Wert SOFORT in den Passwortmanager. **Secret A in Ruhe lassen.**
- [ ] **Daniel — zwei Supabase-Projekte versorgen** (Finanzapp NICHT): SBS `GOOGLE_OAUTH_CLIENT_SECRET` (pltbaqqwpnmdajwgnhpd) · Heineken `GOOGLE_CLIENT_SECRET` (dtdcohqjihoiqlaekeiz). Danach in der SBS-App «Jetzt abgleichen» → Claude verifiziert Logs; Heineken prüft über die stehende `google-secret-probe`.
- [ ] **Daniel — SBS-Gmail prüfen:** Im SBS-Supabase-Dashboard `GMAIL_CLIENT_ID` mit `GOOGLE_OAUTH_CLIENT_ID` vergleichen. Gleich → `GMAIL_CLIENT_SECRET` ebenfalls auf das neue Secret + Testmail. Verschieden → Testmail trotzdem (seit 27.08. gab es laut Logs keinen Versand, noch ist nichts angebrannt).
- [ ] **Daniel — Passwortmanager:** ein Eintrag PRO Client («Google OAuth — <Zweck> (GCP-Projekt <nr>)») mit Client-ID, Secret + Erstelldatum und **Konsumenten-Liste** (welche Supabase-Projekte/Secret-Namen). Rotations-Regel überall: neues Secret → alle Konsumenten laut Liste → verifizieren → **erst dann** altes löschen.
- [ ] Danach: Heinekens `kontakte-google` mittelfristig auf eigenen Client umziehen (1 Client = 1 Verwendung, Vorbild Heineken-Gmail 1040401919292); Governance steht im Memory (`google_oauth_clients.md`) und in den TODO/Decision-Logs der anderen Projekte.
