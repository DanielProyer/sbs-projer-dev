...
- [x] ✓ **Sicherheit: `verify_jwt=true` für `parse-rechnung` + `parse-beleg`** (11.07.2026): via `supabase/config.toml` (`[functions.*] verify_jwt = true`) + Redeploy. Smoke-Test bestätigt: Aufruf **ohne** Auth → `401 UNAUTHORIZED_NO_AUTH_HEADER`, mit gültigem JWT → erreicht die Funktion (App unverändert). Schützt Claude-/API-Credits gegen offene Endpoint-Aufrufe. *(Rest-Hinweis: der öffentliche anon-Key ist ein gültiges JWT und käme durch — für Schutz gegen anon-Key-Abuse bräuchte es zusätzlich einen `auth.getUser()`-Self-Guard in der Funktion; für den Bot-/offene-Endpoint-Fall reicht verify_jwt.)*
