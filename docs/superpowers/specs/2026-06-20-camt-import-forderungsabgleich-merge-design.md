...
2. **Gelernter Zahler-Alias** (`effektiverZahlername` → `betrieb_id` aus Alias-Tabelle) → Betrieb fix, dann Subset-Summe der offenen Forderungen.
...
- Neue Tabelle `camt_zahler_alias` (`user_id`, `zahlername_norm`, `betrieb_id`, `created_at`; unique auf `user_id`+`zahlername_norm`), RLS owner-only.
- **Lernen:** bei jeder manuellen Zuordnung wird `effektiverZahlername(gutschrift)` → `betrieb_id` der gewählten Forderung(en) gespeichert (nur bei **eindeutigem** Betrieb; bei mehreren Betrieben nichts lernen).
- **Anwenden:** Stufe 2 der Matching-Reihenfolge.
- **Verwalten:** kleiner Screen zum Sehen/Löschen gelernter Aliase (falsche Lernung korrigierbar).
...
- **TP-B — Zahler→Betrieb-Lernen:** Alias-Tabelle/Repo; Lernen bei manueller Zuordnung; Anwenden als Matching-Stufe 2; Verwaltungs-Screen.
